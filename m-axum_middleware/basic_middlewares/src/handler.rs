...
use crate::{book::{save_books_to_csv, Book, BookError}, AppState};
...
pub async fn list_books(
    ...
    tracing::info!("Listing all books: {:?}", shared_books);
    tracing::debug!("Total books found: {}", shared_books.len());
    tracing::trace!("Inside list_books handler");
    tracing::warn!("This is a warning log for list_books");
    tracing::error!("This is an error log for list_books");
...
pub async fn get_book(
    ...
    if let Some(book) = shared_books.iter().find(|b| b.id == id) {
        ...
        BookError::NotFound(format!("Book with ID {} not found", id)).into_response()
    }
}
...
// Error Handling - Type 1 - Using `impl Response`
) -> impl IntoResponse {
    if new_book.title.is_none() || new_book.author.is_none() {
        tracing::trace!("Invalid data received for new book: {:?}", new_book);
        return BookError::InvalidData("Title and author are required".to_string()).into_response();
    }

    let mut books_writer = books.write().map_err(|err| {
        // Log error
        tracing::error!("Failed to acquire write lock: {}", err);
        tracing::trace!("Returning server error due to lock acquisition failure");
        // Return a server error if we can't acquire the lock
        BookError::ServerError("Failed to acquire write lock".to_string())
    }).unwrap();
...
// Error Handling - Type 2 - Using `Result<Response, BookError>`
) -> Result<Response, BookError> {
    if updated_book.title.is_none() || updated_book.author.is_none() {
        let error_response =  BookError::InvalidData("Title and author are required".to_string());
    ...
    if found {
        save_books_to_csv(&books_writer).unwrap();
        return Ok((StatusCode::OK, Json(books_writer.clone())).into_response());
    }

    tracing::error!("Book with ID {} not found", id);
    Err(BookError::NotFound(format!("Book with ID {} not found", id)))
}
